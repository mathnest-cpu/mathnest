...
function WorksheetsPanel() {
  const qc = useQueryClient();
  const { user } = useAuth();
  const [form, setForm] = useState({ title: "", topic: "", grade: "5", file: null as File | null });
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("worksheets")
        .select("*, worksheet_assignments(id,grade,student_id)")
        .order("created_at", { ascending: false });
      if (error) throw error;
      return data;
    },
  ...
  const upload = async () => {
    ...
    setUploading(true);
    try {
      const path = `${form.grade}/${Date.now()}-${form.file.name}`;
      const { error: upErr } = await supabase.storage.from("worksheets").upload(path, form.file);
      ...
      // Auto-assign to entire grade
      const { error: aErr } = await supabase.from("worksheet_assignments").insert({
        worksheet_id: ws.id,
        grade: Number(form.grade),
        assigned_by: user!.id,
      });
      if (aErr) throw aErr;
      toast.success("Worksheet uploaded and assigned to grade");
      setForm({ title: "", topic: "", grade: "5", file: null });
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[400px_1fr]">
      <Card className="p-5">
        <h2 className="text-lg font-semibold">Upload worksheet</h2>
        <div className="mt-4 space-y-3">
          <div>
            <Label>Title</Label>
          ...
          <div>
            <Label>Topic</Label>
            <Input placeholder="e.g. Fractions" value={form.topic}
          ...
          <Button className="w-full" disabled={uploading} onClick={upload}>
            <Upload className="mr-2 h-4 w-4" />
            {uploading ? "Uploading…" : "Upload & assign to grade"}
          </Button>
        </div>
      </Card>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={5} className="text-muted-foreground">No worksheets yet.</TableCell></TableRow>
              ) : data!.map((w) => (
                <TableRow key={w.id}>
                  <TableCell className="font-medium">{w.title}</TableCell>
                  <TableCell>Class {w.grade}</TableCell>
                  <TableCell>{w.topic ?? "—"}</TableCell>
                  <TableCell>{format(new Date(w.created_at), "MMM d, yyyy")}</TableCell>
                  <TableCell className="text-right">
                    <Button size="sm" variant="outline" onClick={() => download(w.storage_path, w.file_name)}>
                      Download
                    </Button>
                  </TableCell>
                </TableRow>
